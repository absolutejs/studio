...
var findEntryTarget = (root3, elements, entry) => {
  if (entry.id) return root3.querySelector(`#${CSS.escape(entry.id)}`);
  if (entry.name)
    return root3.querySelector(`[name="${CSS.escape(entry.name)}"]`);
  if (elements[entry.idx]) return elements[entry.idx] ?? null;
  return null;
};
var resolveFocusElement = (root3, elements, activeKey) => {
  if (activeKey.startsWith("id:"))
    return root3.querySelector(
...
var restoreDOMState = (root3, snapshot2) => {
  if (!snapshot2 || !snapshot2.items) return;
  const selector =
    'input, textarea, select, option, [contenteditable="true"], details';
  const elements = root3.querySelectorAll(selector);
  snapshot2.items.forEach((entry) => {
    const target = findEntryTarget(root3, elements, entry);
    if (!target) return;
    restoreEntry(target, entry);
  });
  if (!snapshot2.activeKey) return;
  const focusEl = resolveFocusElement(root3, elements, snapshot2.activeKey);
...
var saveDOMState = (root3) => {
  ...
  if (!active || !root3.contains(active)) return snapshot2;
...
var restoreDOMChanges = (root3, snapshot2, newHTML) => {
  const tempDiv = document.createElement("div");
  tempDiv.innerHTML = newHTML;
  snapshot2.text.forEach((liveText, elId) => {
    ...
    const liveEl = root3.querySelector(`#${CSS.escape(elId)}`);
    if (liveEl) {
...
var snapshotDOMChanges = (root3) => {
  const text2 = new Map();
  const children = new Map();
  root3.querySelectorAll("[id]").forEach((elem) => {
...
// src/frontend/svelte/indexes/pages/SvelteExample.js
...
if (isHMR) {
  ...
  component2 = mount(SvelteExample, {
    target: document.body,
    props: mergedProps,
  });
  window.__HMR_PRESERVED_STATE__ = undefined;
} else {
  component2 = hydrate(SvelteExample, {
    target: document.body,
    props: initialProps,
  });
}
