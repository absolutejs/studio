...
const printBanner = (studioPort: number, devPort: number, duration: number) => {
	...
	console.log()
	console.log(`  ${GREEN}➜${RESET}  ${BOLD}Studio:${RESET}      http://localhost:${studioPort}`)
	console.log(`  ${GREEN}➜${RESET}  ${BOLD}Dev server:${RESET}  http://localhost:${devPort}`)
	console.log()
}
...
const printHelp = () => {
	...
	console.log(`  ${DIM}r${RESET}  restart    ${DIM}\u2014 restart the dev server${RESET}`)
	console.log(`  ${DIM}o${RESET}  open       ${DIM}\u2014 open studio in browser${RESET}`)
	console.log(`  ${DIM}c${RESET}  clear      ${DIM}\u2014 clear the terminal${RESET}`)
	console.log(`  ${DIM}q${RESET}  quit       ${DIM}\u2014 shut down the studio${RESET}`)
	console.log(`  ${DIM}h${RESET}  help       ${DIM}\u2014 show this help${RESET}`)
	console.log()
}
...
const openBrowser = async (url: string) => {
	...
	try {
		...
		console.log(`  ${YELLOW}Could not open browser. Visit: ${CYAN}${url}${RESET}`)
	}
}

const killPort = async (port: number) => {
	try {
		const proc = Bun.spawn(['sh', '-c', `lsof -ti:${port} | xargs kill 2>/dev/null`], {
			stdout: 'ignore',
			stderr: 'ignore',
		})
		await proc.exited
...
const main = async () => {
	...
	const { projectDir, reactDirectory, devEntry, devConfig, port, devPort } = parseArgs()
	...
	const spawnDevServer = () => {
		...
		const readStream = async (stream: ReadableStream<Uint8Array>, target: NodeJS.WriteStream) => {
		...
		process.stdin.on('data', async (key: string) => {
			...
			if (key === '\r' || key === '\n') {
				...
				switch (cmd) {
					...
					default: {
						if (cmd) {
							console.log(`  ${DIM}Unknown command: "${cmd}". Press h for help.${RESET}`)
						}
					}
				}
			} else {
